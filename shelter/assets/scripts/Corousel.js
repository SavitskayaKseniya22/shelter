    ...
    addListener() {
        document.addEventListener("click", (event) => {
            if (event.target.closest(".friends-corousel-button__right")) {
                ...
                if (this.sequence.slice(-2) == "<>") {
                    this.nextCards = this.buffer[this.buffer.length - 1];
                } else {
                ...
                this.buffer.push(this.activeCards);

                document.querySelector(
                    ".friends-list__active"
                ).style = `transform: translateX(-${this.offset}); transition: transform 1s`;
                document.querySelector(
                    ".friends-list__next"
                ).style = `transform: translateX(-${this.offset}); transition: transform 1s`;
                ...
                if (this.sequence.slice(-2) == "><") {
                    this.previousCards = this.buffer[this.buffer.length - 1];
                } else {
